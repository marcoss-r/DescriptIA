...
function rlActualizarControles() {
  ...
  // TODO 6.1 — Con un if/else if (o switch) sobre rlEstado.subEstado, enciende lo
  // que corresponde:
  //   - "pista":      ocultar.hidden = false;
  //                   ocultar.textContent = `Ocultar y pasar a ${nombre}`;
  //   - "adivinando": confirmar.hidden = false;
  //   - "resultado":  siguiente.hidden = false;
  //                   const puntos = rlCalcularPuntos(rlEstado.anguloAguja, rlEstado.centroSector);
  //                   resultado.textContent = `+${puntos} puntos para ${nombre}`;
...
function rlSiguiente() {
  // TODO 6.2 — Doble rotación (jugador dentro de temática, temática dentro de partida):
  //   1. Avanza al siguiente jugador con módulo (rotación en círculo):
  //        rlEstado.turnoIndex = (rlEstado.turnoIndex + 1) % rlEstado.jugadores.length;
  //   2. Si turnoIndex volvió a 0, TODOS adivinaron esta temática → pasa a la
  //      siguiente: rlEstado.tematicaIndex++;
  //      y si ya no quedan temáticas (tematicaIndex >= rlEstado.tematicas.length),
  //      la partida termina: llama a rlTerminar() y haz `return`.
  //   3. Si la partida sigue, prepara el siguiente turno con rlIrATurno().
  rlEstado.turnoIndex = (rlEstado.turnoIndex + 1) % rlEstado.jugadores.length;
...
// Fin de la partida: muestra la pantalla final (el ranking se pinta en la Fase 7).
function rlTerminar() {
